...
class Bienvenida_3(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.imágenes_flechas=(
            pygame.image.load(os.path.join(dirImágenes,'tecla_der.png')),
    ...
    def dibujar(self,superficie):
        ...
        self.texto.mostrar_texto('Ayuda a Cody a limpiar su casa de las arañas!',33,color_banner,ancho_ventana//2+2,10+2,superficie,None)
        self.texto.mostrar_texto('Ayuda a Cody a limpiar su casa de las arañas!',33,color_techo,ancho_ventana//2,10,superficie,None)
        self.texto.mostrar_texto('Con las flechas laterales mueves a Cody',33,color_banner,ancho_ventana//2+2,60+2,superficie,None)
        self.texto.mostrar_texto('Con las flechas laterales mueves a Cody',33,color_techo,ancho_ventana//2,60,superficie,None)
        self.texto.mostrar_texto('Presiona cualquier tecla para continuar',33,color_banner,ancho_ventana//2+2,110+2,superficie,None)
    ...
    def animación(self,imágenes,rectángulo,superficie):
        ...
        while esperar:
            ...
            for i in range(40):
                ...
                superficie.blit(self.imagen_piso,self.rect_piso)
                self.piso.dibujar(superficie)
                self.jugador.izquierda(velocidad_jugador)
                self.jugador.dibujar(superficie,True)
                superficie.blit(imágenes[1],rectángulo)
                time.wait(15)
            for i in range(40):
                pygame.display.flip()
                superficie.blit(self.imagen_piso,self.rect_piso)
                self.piso.dibujar(superficie)
                self.jugador.derecha(velocidad_jugador)
                self.jugador.dibujar(superficie,True)
